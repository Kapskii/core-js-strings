...
function isPalindrome(str) {
  const newStr = str.replace(/[^\w]/g, '').split('').join('');
  const revStr = str.replace(/[^\w]/g, '').split('').reverse().join('');
  return revStr.toLowerCase() === newStr.toLowerCase();
}
...
function reverseWords(str) {
  ...
  const reverseWord = word.map((el) => el.split('').reverse().join(''));
